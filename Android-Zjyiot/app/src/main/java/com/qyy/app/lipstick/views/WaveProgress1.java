...
import com.qyy.app.lipstick.utils.Constant;
...
public class WaveProgress1 extends View {

    private static final String TAG = WaveProgress1.class.getSimpleName();
    ...
    public WaveProgress1(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }
    DecimalFormat fnum;
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.WaveProgress1);

        antiAlias = typedArray.getBoolean(R.styleable.WaveProgress1_antiAlias, true);
        mDarkWaveAnimTime = typedArray.getInt(R.styleable.WaveProgress1_darkWaveAnimTime, Constant.DEFAULT_ANIM_TIME);
        mLightWaveAnimTime = typedArray.getInt(R.styleable.WaveProgress1_lightWaveAnimTime, Constant.DEFAULT_ANIM_TIME);
        mMaxValue = typedArray.getFloat(R.styleable.WaveProgress1_maxValue, Constant.DEFAULT_MAX_VALUE);
        mValue = typedArray.getFloat(R.styleable.WaveProgress1_value, Constant.DEFAULT_VALUE);
        mValueSize = typedArray.getDimension(R.styleable.WaveProgress1_valueSize, Constant.DEFAULT_VALUE_SIZE);
        mValueColor = typedArray.getColor(R.styleable.WaveProgress1_valueColor, Color.BLACK);

        mHint = typedArray.getString(R.styleable.WaveProgress1_hint);
        mHintColor = typedArray.getColor(R.styleable.WaveProgress1_hintColor, Color.BLACK);
        mHintSize = typedArray.getDimension(R.styleable.WaveProgress1_hintSize, Constant.DEFAULT_HINT_SIZE);

        mCircleWidth = typedArray.getDimension(R.styleable.WaveProgress1_circleWidth, Constant.DEFAULT_ARC_WIDTH);
        mCircleColor = typedArray.getColor(R.styleable.WaveProgress1_circleColor, Color.GREEN);
        mBgCircleColor = typedArray.getColor(R.styleable.WaveProgress1_bgCircleColor, Color.WHITE);

        mWaveHeight = typedArray.getDimension(R.styleable.WaveProgress1_waveHeight, Constant.DEFAULT_WAVE_HEIGHT);
        mWaveNum = typedArray.getInt(R.styleable.WaveProgress1_waveNum, 1);
        mDarkWaveColor = typedArray.getColor(R.styleable.WaveProgress1_darkWaveColor,
                getResources().getColor(android.R.color.holo_blue_dark));
        mLightWaveColor = typedArray.getColor(R.styleable.WaveProgress1_lightWaveColor,
                getResources().getColor(android.R.color.holo_green_light));

        isR2L = typedArray.getInt(R.styleable.WaveProgress1_lightWaveDirect, R2L) == R2L;
        lockWave = typedArray.getBoolean(R.styleable.WaveProgress1_lockWave, false);
    ...
    private void initPaint() {
        ...
        mPercentPaint1 = new Paint();
        mPercentPaint1.setTextAlign(Paint.Align.CENTER);
        mPercentPaint1.setAntiAlias(antiAlias);
        mPercentPaint1.setColor(mValueColor);
        mPercentPaint1.setTextSize(mValueSize/3);
    }
    ...
    private void drawProgress(Canvas canvas) {
        ...
        float y1 = mCenterPoint.y -(mPercentPaint.descent() + mPercentPaint.ascent())/2 ;
        float y3 = mCenterPoint.y -(mPercentPaint.descent() + mPercentPaint.ascent())*3/2 ;
...
//            Log.d(TAG, "mPercent = " + mPercent + "; mPrePercent = " + mPrePercent);
        ...
        canvas.drawText("已用"+  fnum.format(useFlow)+"M",mCenterPoint.x,y1,mPercentPaint1);
        if ((totalFlow-useFlow)<0){
            canvas.drawText("剩余"+ fnum.format(0)+"M",mCenterPoint.x,y3,mPercentPaint1);
        }else {
            canvas.drawText("剩余"+ fnum.format((totalFlow-useFlow))+"M",mCenterPoint.x,y3,mPercentPaint1);
        }
         if (mHint != null) {
